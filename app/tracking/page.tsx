...
// Add type declaration for getElementById
declare global {
  interface Document {
...
// URL Google Apps Script Web App (ganti dengan URL Apps Script Anda)
...
const TrackingPage = () => {
  ...
  const fetchAllDocuments = async () => {
    try {
      setIsLoading(true);
      setDebugInfo("Fetching all documents...");
      ...
      if (data && data.success && data.documents && Array.isArray(data.documents)) {
        ...
        setDataLoaded(true);
        setDebugInfo(prev => prev + "\nProcessed " + data.documents.length + " documents");
      } else {
  ...
  const handleSearch = async (e: React.FormEvent | null) => {
    ...
    try {
      ...
      if (localResults.length > 0) {
        ...
        setDebugInfo(prev => prev + "\nAPI response: " + JSON.stringify(data).substring(0, 100) + "...");
  ...
  const getDocumentDetails = (doc: Document | null): DocumentDetail[] => {
    if (!doc) return [];
  ...
  return (
    <MainLayout>
      {process.env.NODE_ENV === 'development' && false && (
        <div className="fixed bottom-0 right-0 p-4 bg-black text-green-400 opacity-75 max-w-xl max-h-64 overflow-auto text-xs">
                ...
                <div className="mb-6">
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      placeholder={searchType === 'nama' ? "Contoh: Eka Krisna Santoso" : "Contoh: 19860708"}
                      value={searchQuery}
      ...
      <section className="py-16 bg-gradient-to-b from-dark-bg to-fsti-dark">
        <div className="container mx-auto px-4 text-center">
          <AnimatedSection animation="slideUp">
            <h2 className="text-2xl font-display font-bold mb-6 text-white">Coba Contoh Pencarian</h2>
            <p className="text-gray-300 mb-8 max-w-2xl mx-auto">
              Ingin mencoba sistem tracking? Gunakan contoh nama di bawah ini:
            </p>
            <div className="flex flex-wrap justify-center gap-4">
              {dataLoaded && allDocuments.length > 0 ? (
                // Tampilkan beberapa nama unik untuk contoh
                Array.from(new Set(allDocuments.slice(0, 6).map(doc => doc.requestor)))
                  .filter(Boolean)
                  .slice(0, 3)
                  .map((name, index) => (
                    <Button 
                ...
                <div className="text-center text-gray-400 mt-4">
                  <RotateCw className="w-5 h-5 inline-block mr-2 animate-spin" />
                  Memuat contoh data...
                </div>
              )}
            </div>
